...
collapsed_bed <- collapsed %>% 
    mutate(chrom = 'U00096.2',
           name = '',
           score = '',
           collapse_start = peak_start + tile_start,
           collapse_end = peak_start + tile_end) %>% 
    select(chrom, collapse_start, collapse_end, name, score, strand)
```

```{r}
# grab mean and median expression for collapsed tiles
extract_expression <- function(collapse_start, collapse_end, collapse_tile_start, collapse_tile_end, buffer, df) {
...
summary_exp <- apply(collapsed_bed, 1, function(x) extract_expression(collapse_start=as.numeric(x[10]),
                                                                      collapse_end=as.numeric(x[11]),
                                                                      collapse_tile_start=as.numeric(x[3]),
                                                                      collapse_tile_end=as.numeric(x[4]),
                                                                      buffer=200,
                                                                      df=active))
collapsed_bed$mean_exp <- summary_exp[1,]
collapsed_bed$median_exp <- summary_exp[2,]
```

```{r}
write.table(select(collapsed_bed, chrom, collapse_start, collapse_end, name, median_exp, strand), 
