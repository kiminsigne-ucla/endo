...
if(length(filelist) == 8) {
    expression <- mapped_bc_counts %>% 
        ...
        ungroup() %>%
        select(variant, orig_name = name, RNA_exp_1_1, RNA_exp_1_2, RNA_exp_2_1, RNA_exp_2_2,
...
neg_sd <- sd(negatives$RNA_exp_ave)
neg_median <- median(negatives$RNA_exp_ave, na.rm = T)
threshold <- 2*neg_sd + neg_median
print(paste("Active threshold:", threshold))
print("Standardize so threshold = 1")

expression <- expression %>% 
    mutate(expn_std = RNA_exp_ave - threshold)
